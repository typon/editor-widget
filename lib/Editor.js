...
Editor.prototype.clipScroll = function (poss) {
  ...
  return self;
    ...
    Editor.prototype._markRows = function () {
        var self = this;
        self.destroyMarkers({type: 'marked'});
        //for self.rowMarkers
        for (const row of Object.keys(self.rowMarkers)) {
            var rowRange = self.textBuf.rangeForRow(row, true)
            self.textBuf.markRange(rowRange, {type: 'marked'});
        }
        return self;
    };
...
Editor._nonprintableRegExp = /[\x00-\x1f]|\x7f/g;
    ...
    Editor.MARKER_ORDER = ['syntax', 'match', 'findMatch', 'selection', 'marked'];
...
Editor.prototype._updateContent = function () {
  ...
  var markedRowStyle = style.markedRow;
  ...
  util.text.splitLines(BaseWidget.blessed.escape(self.textBuf.getTextInRange({
      ...
      if (range.intersectsRow(row)) {
        ...
        switch (marker.properties.type) {
          ...
          case 'syntax': markerStyle = marker.properties.syntax
            ...
            .join(''); break;
          case 'marked': markerStyle = markedRowStyle; break;
    ...
    bufferContent.push(line + '{/}');
            ...
            var gutterLineMarker = '';
            if (self.rowMarkers[row] !== undefined)  {
                gutterLineMarker = self.rowMarkers[row];
            }
            var gutterLineText = (row + 1) + gutterLineMarker;
            var gutterLine = _.padLeft(gutterLineText, lineNumberWidth) + _.repeat(' ', gutterWidth);
